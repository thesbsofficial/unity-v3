...
# Handles both Cloudflare proxying and local item database management
    ...
    while ($listener.IsListening) {
        ...
        try {
            ...
            switch -Regex ($url) {
                ...
                "^/api/items/([^/]+)$" {
                    # --- Single Item Operations (PUT for updates) ---
                    $itemId = $matches[1]
                    
                    if ($method -eq "PUT") {
                        Write-Host "  Updating item $itemId in local database..." -ForegroundColor Yellow
                        $body = New-Object System.IO.StreamReader($request.InputStream) | ForEach-Object { $_.ReadToEnd() }
                        Write-Host "  Request body: $($body.Substring(0, [Math]::Min(100, $body.Length)))" -ForegroundColor Gray
                        ...
                        if ($itemIndex -ge 0) {
                            ...
                            if ($updateData.tags) {
                                $existingItem.tags = $updateData.tags
                            }
                            if ($updateData.status) {
                                $existingItem.status = $updateData.status
                            }
                            if ($updateData.notes) {
                                $existingItem.notes = $updateData.notes
                            }
                            
                            # Always update the timestamp
                            $existingItem.updatedAt = (Get-Date).ToString("o")
                            
                            # Save back to file
                            if (Set-JsonData -filePath $itemsFile -data $items) {
                                Add-AuditEntry -action "UPDATE" -itemId $itemId -details @{ 
                                    fields = ($updateData.PSObject.Properties.Name -join ", ")
                                    tags = $existingItem.tags -join ", "
                                }
                                $responseBody = @{ success = $true; item = $existingItem }
                                Write-Host "  ✅ Item $itemId updated successfully" -ForegroundColor Green
                            } else {
                                throw "Failed to save updated item to database"
                            }
                        } else {
                            $statusCode = 404
                            $responseBody = @{ success = $false; error = "Item not found: $itemId" }
                ...
                "^/api/test$" {
                    ...
                    Write-Host "  Processing connection test..." -ForegroundColor Yellow
                    $responseBody = @{ 
                        success = $true
                        server = "SBS Media Hub API Server"
                        version = "1.0.0"
                        timestamp = (Get-Date).ToString("o")
                        port = $Port
                        status = "Connected"
                        endpoints = @("/api/images", "/api/items", "/api/items/{id}", "/api/test")
                    }
                    Write-Host "  ✅ Connection test successful" -ForegroundColor Green
                }
                "^/api/items/stats$" {
                    # --- Database Statistics Endpoint ---
                    Write-Host "  Loading database statistics..." -ForegroundColor Yellow
                    $items = Get-JsonData -filePath $itemsFile
                    if ($items -isnot [array]) { $items = @() }
                    
                    $availableCount = ($items | Where-Object { $_.imageStatus -eq "available" }).Count
                    $deletedCount = ($items | Where-Object { $_.tags -contains "DELETED" }).Count
                    $categories = ($items | Select-Object -ExpandProperty category -Unique).Count
                    ...
                    Write-Host "  ✅ Statistics loaded: $($items.Count) total items" -ForegroundColor Green
                }
                default {
                    $statusCode = 404
